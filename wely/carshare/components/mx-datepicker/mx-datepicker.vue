	...
	<view v-if="isShow" class="picker">
		...
		<view v-if="type!='time'" class="picker-modal">
			...
			<view class="picker-modal-footer">
				...
				<view class="picker-modal-footer-btn">
					<view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancel">クリア</view>
					<view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirm">確定</view>
				</view>
			</view>
		</view>
		<!-- 时间选择器 -->
		<view v-if="showTimePicker" class="picker">
			<view class="picker-modal picker-time">
				<view class="picker-modal-header">
					<text class="picker-modal-header-title">选择期日です</text>
				</view>
				...
				<view class="picker-modal-footer">
					...
					<view class="picker-modal-footer-btn">
						<view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active" @click="onCancelTime">クリア</view>
						<view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active" @click="onConfirmTime">確定</view>
					</view>
				</view>
			</view>
		</view>
		...
		methods: {
			...
			procCalendar(item) {
				...
				if (this.checkeds.length == 2) {
					if (DateTools.isSameDay(this.checkeds[0], item.dateObj)) { //开始期日です
						item.bgStyle.type = 'bgbegin';
					}
					if (DateTools.isSameDay(this.checkeds[1], item.dateObj)) { //结束期日です
						if (this.isMultiSelect && this.showTips) item.tips = item.bgStyle.type ? this.beginText + ' / ' + this.endText : this.endText;
						if (!item.bgStyle.type) { //开始期日です不等于结束期日です
							item.bgStyle.type = 'bgend';
						} else {
							item.bgStyle.type = '';
						}
					}
					if (!item.bgStyle.type && (+item.dateObj > +this.checkeds[0] && +item.dateObj < +this.checkeds[1])) { //中间的期日です

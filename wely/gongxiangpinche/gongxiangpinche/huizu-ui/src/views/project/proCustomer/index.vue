...
<!--      </el-table-column>-->
      <el-table-column label="アイコン" align="center" prop="avatar" width="100">
        <template slot-scope="scope">
          <image-preview v-if="scope.row.avatar != null" :src="scope.row.avatar" :width="50" :height="50"/>
        </template>
      </el-table-column>
...
<!--        </el-form-item>-->
        ...
        <el-form-item label="自己紹介" prop="remark">
          <el-input v-model="form.remark" type="textarea" placeholder="一度入力してください内容"  maxlength="500" show-word-limit style="width:580px"/>
        </el-form-item>
      </el-form>
...
<!--        </el-descriptions-item>-->
      ...
      <el-descriptions class="margin-top" :column="1" border :labelStyle="labelStyle">
        <el-descriptions-item label="アイコン">
          <image-preview :src="formDetails.avatar" :width="50" :height="50"/>
        </el-descriptions-item>
<!--        <el-descriptions-item label="名乗する">-->
<!--          <p class="wrapper" v-html='formDetails.content'></p>-->
<!--        </el-descriptions-item>-->
        <el-descriptions-item label="運転免許証">
          <image-preview v-if="formDetails.drivingLicense!=null && formDetails.drivingLicense!=''" :src="formDetails.drivingLicense" :width="50" :height="50"/>
        </el-descriptions-item>
        <el-descriptions-item label="車検証">
          <image-preview v-if="formDetails.insurance!=null && formDetails.insurance!=''" :src="formDetails.insurance" :width="50" :height="50"/>
        </el-descriptions-item>
        <el-descriptions-item label="自動車保険証">
          <image-preview v-if="formDetails.insuranceCertificate!=null && formDetails.insuranceCertificate!=''" :src="formDetails.insuranceCertificate" :width="50" :height="50"/>
        </el-descriptions-item>
    ...
    <el-dialog :title="title" :visible.sync="openAuditDetails" width="900px" append-to-body>
      <el-descriptions class="margin-top" :column="1" border :labelStyle="labelStyle">
        <el-descriptions-item label="認証の詳細">
          <image-preview :src="formDetails.numberImage" :width="50" :height="50"/>
        </el-descriptions-item>
      </el-descriptions>
    </el-dialog>
    <!--提现信息-->
    <el-dialog :title="title" :visible.sync="openWithdraw" width="900px" append-to-body>
      <el-descriptions class="margin-top" :column="2" border :labelStyle="labelStyle">
        ...
        <el-descriptions-item label="实际提现金额">
...
export default {
  name: "ProCustomer",
  dicts: ['sys_open_status', 'pro_customer_type', 'sys_user_sex', 'yes_no'],
  data() {
    return {
  ...
  methods: {
    ...
    handleWithdraw(row) {
      ...
      getProCustomer(id).then(response => {
        this.formDetails = response.data;
        this.formDetails.realIntegral = (this.formDetails.integral*0.8).toFixed(2);
        this.formDetails.brokerage = (this.formDetails.integral*0.2).toFixed(2);
        getProCustomerCardByCustomerId(id).then(res => {
          this.formDetails.cardNo = res.data.cardNo
          this.formDetails.expire = res.data.expire
          this.formDetails.name = res.data.name
          this.openWithdraw = true
          this.title = "キャッシュバック審査です";
        });
      });
    },
    /** 导出按钮操作 */

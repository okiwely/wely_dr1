<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['project:tbCustomerChat:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['project:tbCustomerChat:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['project:tbCustomerChat:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['project:tbCustomerChat:export']"
        >导出</el-button>
      </el-col>
    ...
    <!-- 聊天信息表格数据列表 -->
    <el-table v-loading="loading" :data="tbCustomerChatList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="评论人id" align="center" prop="customerId" />
      <el-table-column label="被评论人id" align="center" prop="subId" />
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['project:tbCustomerChat:details']"
          >详情</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="openDetails" width="900px" append-to-body>
      <el-descriptions class="margin-top" :model="formDetails" :column="2" border :labelStyle="labelStyle" :contentStyle="contentStyle">
        <el-descriptions-item label="评论人id">
          {{ formDetails.customerId }}
        </el-descriptions-item>
        <el-descriptions-item label="被评论人id">
          {{ formDetails.subId }}
        </el-descriptions-item>
...
<script>
import { listTbCustomerChat, getTbCustomerChat, delTbCustomerChat, addTbCustomerChat, updateTbCustomerChat } from "@/api/project/tbCustomerChat";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTbCustomerChat(this.queryParams).then(response => {
        this.tbCustomerChatList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTbCustomerChat(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateTbCustomerChat(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTbCustomerChat(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除聊天信息编号为"' + ids + '"的数据项？').then(function() {
        return delTbCustomerChat(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      this.$modal.confirm('是否根据搜索条件导出所有数据？').then(() => {
        this.download('project/tbCustomerChat/export', {
          ...this.queryParams
        }, `聊天信息_${new Date().getTime()}.xlsx`)
      })
    }
  }
};

				...
				<view style="margin: 2.372rem auto;width:104.98rem;">
					...
					<view class="flexJusp" v-for="(item,index) in list" :key="index"
						...
						<view v-if="item.orderStatus==4"
							style="width: 17.233rem;height:20.316rem;background: #F7F8F9;border-radius:  1.581rem;padding: 1.581rem  3.162rem;">
					       <view  class="buttons" style="background:#cccccc;margin: 6.087rem 0 0;">レビュー</view>
						</view>
					</view>
				...
				<view style="margin: 2.372rem auto;width:calc(100vw - 4.74rem);">
					...
					<view class="flexJusp" v-for="(item,index) in list" :key="index"
						...
						<view
							...
							<view class="flex" @click='topersonal(item)'>
								<image style="width: 10.91rem;height:10.91rem;border-radius:50%;" :src="item.avatar && item.avatar.replace('wely.info', '54.92.100.72:9006')">
								</image>
						...
						<view v-if="item.orderStatus==4"
							...
							<!-- 	<view @click="details(item)" class='detail'>詳細</view>
							<view @click="jiaru(item)" class='send' >リクエスト送信</view> -->
							<view class="detail" style='background: #CCCCCC;margin:0 auto;width: 55.23rem;border:none;color:#fff'>レビュー</view>
						</view>
						<!-- <view v-if="item.orderStatus==1"
							style="width: 17.233rem;height:20.316rem;background: #F7F8F9;border-radius:  1.581rem;padding: 1.581rem  3.162rem;">
							<view @click="zhifu(item)" class="buttons" style="margin: 6.087rem 0 0;">お支払い</view>
	...
	export default {
		...
		methods: {
			topersonal(item) {
				...
				if (item.orderStatus != 1 && item.orderStatus != 3 && item.orderStatus != 4) {
					return
				}
				uni.navigateTo({
			...
			loads() {
				...
				if (this.type == 1) {
					...
					Promise.all([
						...
						// });
                       const storedArray = uni.getStorageSync('finishArray') || [];
						// 过滤掉重复 id 的项
						...
						// this.list = storedArray.map(item => {
						//   const match = this.list.find(listItem => listItem.id === item.id);
						//   if (match) {
						//     return { ...item, orderStatus: 4 };
						//   }
						//   return item;
						// });
							console.log(storedArray.includes())
						this.list = this.list.map(item => {
						  // 如果 item.id 存在于 storedArray 中
						  if (storedArray.includes(item.id+'')) {
						    // 返回一个新的对象，更新 orderStatus 为 4
							console.log(111)
						    return { ...item, orderStatus: "4" };
						  }
						  // 如果没有匹配，保持原 item 不变
			...
			pingjia(item) {
				uni.navigateTo({
					url: '/pages/index/evaluate/evaluate?subId=' + item.subId + '&orderCode=' + item.orderCode+'&id=' + item.id
				})
			},
			submitPayment() {
				var that = this

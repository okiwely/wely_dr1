<template>
	<view>
		<view class="content">
			<headers ref="headers" canGoBack title='リクエスト一覧ページ' style="position: fixed;top:0;left:0;z-index: 1;"></headers>
			<view class='pc-only'>
				...
				<view class="flex" style="margin: 2.372rem auto;width:139.13rem;">
					<view style="width:30.988rem;">
						<view style="font-weight: bold;font-size: 1.897rem;color: #000000;line-height:1.897rem"><text
								style="cursor: pointer;" @click="indexx">TOPペ—ジ>></text><text
								style="color: #FF3D66;">リクエスト一覧ページ</text></view>
						<view
							...
							<view class="flexJusp" style="align-items: center;margin:1.265rem 0 1.897rem;">
								<!-- <picker mode="date" :value="date" @change="bindDateChange">
										<view class="flexJusp"
											style="width:19.447rem;height: 3.4rem;border-radius: 7px;border: 1px solid #707070;align-items: center;padding:0 0.948rem;">
											<view>{{date}}</view>
											<image style="width: 1.739rem;height:1.818rem;" src="/static/images/mnvjyt.png"></image>
										</view>
									</picker> -->
					...
					<view style="width:104.98rem;margin:5.85rem 0 0  3.162rem;">
						<view class="flexJusp" style="margin-bottom:2.213rem;">
							<view class="flex" style="font-weight: bold;font-size:  1.739rem;">
								<view>{{page.total}}件</view>
								<view style="margin-left: 3.162rem;">のリクエストが見つかりました。</view>
							</view>
							<view style='flex:1'></view>
							<picker @change="bindPickerChangeSort" :value="indexSort" :range="listSort"
								range-key="city">
								<!-- 	<view class="flexJusp"
										style="width:25.929rem;height: 3.636rem;border-radius: 1.106rem;border: 1px solid #707070;align-items: center;padding:0 0.948rem;margin:1.265rem 0 1.897rem;">
							...
							<!-- 	<view class="flexJusp" @click="paixu"
									style="width:21.344rem;height: 3.478rem;border-radius: 1.106rem;border: 1px solid #888888;align-items: center;padding:0 0.948rem;cursor: pointer;">
									<view style="font-size:1.423rem;">出発日時の早い順</view>
									<image v-if="sort==1" style="width:1.265rem;height:1.265rem;" src="/static/images/downs.png">
									</image>
									<image v-if="sort==2" style="width:1.265rem;height:1.265rem;" src="/static/images/ups.png">
									</image>
								</view> -->
						</view>
						<view class="flexJusp" v-for="(item,index) in list" :key="index"
							...
							<view
								style="width: 26.798rem;height:16.522rem;background: #F7F8F9;border-radius:  1.581rem;padding: 1.581rem;">
								<view class="flex" style="margin-top:4.348rem;" @click='topersonal(item)'>
									<image style="width: 10.869rem;height:10.869rem;border-radius:50%;"
										:src="item.avatar && item.avatar.replace('wely.info', '54.92.100.72:9006')">
									</image>
									<view style="margin-left:1.107rem;">
										...
										<view v-if="item.score!=0" class="flex"
											...
											<image style="width: 1.739rem;height: 1.581rem;"
												src="/static/images/bxs.png">
											</image>
											<view style="font-size: 1.423rem;color: #F88400;margin:0 0.948rem 0 6px;">
												{{Math.floor(item.score * 10) / 10}}
											</view>
											<view style="font-size:  1.581rem;color: #333333;"></view>
										</view>
							...
							<view
								style="width: 44.269rem;height: 16.522rem;background: #F7F8F9;border-radius:  1.581rem;padding: 1.581rem;">
							...
							<view
								style="width: 17.233rem;height: 16.522rem;background: #F7F8F9;border-radius:  1.581rem;padding: 1.581rem  3.162rem;">
								<view @click="details(item)"
									style="width: 17.233rem;height: 5.06rem;background: #F44870;border-radius: 1.264rem;font-weight: bold;font-size:  1.739rem;color: #FFFFFF;line-height: 5.06rem;text-align: center;margin:5rem 0  2.688rem;cursor: pointer;">
									詳細</view>
								<!-- <view
						...
						<view class="flexJusp" v-for="(item,index) in list" :key="index"
							...
							<view
								...
								<view class="flex" @click='topersonal(item)'>
									<image style="width: 10.91rem;height:10.91rem;border-radius:50%;"
										:src="item.avatar && item.avatar.replace('wely.info', '54.92.100.72:9006')">
									</image>
									<view style="margin-left:2.05rem;margin-top:-0.68rem">
										...
										<view class="flex" style="align-items: center;margin:0.91rem 0 0.91rem 0;"
											...
											<view style="font-size: 2.73rem;color: #F88400;">{{Math.floor(item.score * 10) / 10}}</view>
											<view style="font-size: 2.73rem;color: #333333;"></view>
										</view>
									...
									<view style='flex:1'></view>
									<view style="font-size: 3.18rem;color: #F44870;margin-right:2.27rem">
							...
							<view
								...
								<view @click="details(item)" class='send' style='margin:0 auto'>詳細</view>
			...
			<!-- 	<view class="flex" style="margin: 2.372rem auto;width:139.13rem;">
				<view style="width:30.988rem;">
					<view style="font-weight: bold;font-size: 1.897rem;color: #000000;line-height:1.897rem"><text
							style="cursor: pointer;" @click="indexx">TOPペ—ジ>></text><text
							style="color: #FF3D66;">リクエス一覧ページ</text></view>
					<view
						style="margin-top:3.873rem;width: 27.826rem;background: #FFFFFF;box-shadow: 0px 6px 1.264rem 1px rgba(0,0,0,0.06);border-radius:  1.581rem;padding:1.897rem  1.581rem  2.372rem;">
				...
				<view style="width:104.98rem;margin:5.85rem 0 0  3.162rem;">
					<view class="flexJusp" style="margin-bottom:2.213rem;">
						...
						<picker @change="bindPickerChangeSort" :value="indexSort" :range="listSort" range-key="city">
							...
							<view class="flexJusp"
								style="width:21.344rem;height: 3.478rem;border-radius: 1.106rem;border: 1px solid #888888;align-items: center;padding:0 0.948rem;cursor: pointer;">
								<view style="font-size:1.423rem;">{{listSort[indexSort]}}</view>
								<image style="width:1.265rem;height:1.265rem;" src="/static/images/downs.png"></image>
							</view>
						</picker>
						<view class="buttonss" @click="addride">リクエスト登録</view>
					...
					<view class="flexJusp" v-for="(item,index) in list" :key="index"
						...
						<view
							style="width: 26.798rem;height:16.522rem;background: #F7F8F9;border-radius:  1.581rem;padding: 1.581rem;">
							<view class="flex" style="margin-top:4.348rem;" @click='topersonal(item)'>
								<image style="width: 10.869rem;height:10.869rem;border-radius:50%;" :src="item.avatar && item.avatar.replace('wely.info', '54.92.100.72:9006')">
								</image>
								<view style="margin-left:1.107rem;">
									...
									<view v-if="item.isAuth==1" style="font-size: 1.264rem;color: #666666;">本人確認書類提出済み
									</view>
								</view>
							</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			loads() {
				...
				this.$httpapi('project/getTbCustomerRouteListU', 'POST', {
			...
			details(item) {
				uni.navigateTo({
					url: '/pages/index/userList/userDetails?id=' + item.id
				})
			}
		}
	}

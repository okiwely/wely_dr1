    ...
    public AjaxResult getTbCustomerRouteListNow(JSONObject jsonObject)
    {
        ...
        for (JSONObject item: jsonObjectList){
            ...
            if (orderCode != null && orderCode.length()>0){
                ProOrderInfo proOrderInfo = orderInfoMapper.selectProOrderInfoByCode(orderCode);
                ProCustomer proCustomer = customerMapper.selectProCustomerById(proOrderInfo.getDriverId().longValue());
                item.put("avatar",proCustomer.getAvatar());
                item.put("name",proCustomer.getName());
                item.put("score",proCustomer.getScore());
                item.put("content",proCustomer.getContent());
            }
        }
        return AjaxResult.success(jsonObjectList);
    }
    ...
    public AjaxResult calculatePrice(JSONObject jsonObject)
    {
        ...
        //设置司机相关参数
            ...
            if (car != null && startLonLat != null && endLonLat != null){
                ...
                //添加20%手续费，最终价格
                BigDecimal price = km.divide(oilLoss,4,BigDecimal.ROUND_UP)
                                    .multiply(oilPrice)
                                    .add(freeway)
